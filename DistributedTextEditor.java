...
public class DistributedTextEditor extends JFrame {
	...
	public DistributedTextEditor() {
		...
		er = new EventReplayer(dec, area1,area2,this);
		ert = new Thread(er);
		ert.start();
	}
	...
	Action Listen = new AbstractAction("Listen") {
		public void actionPerformed(ActionEvent e) {
			...
			area1.setText("");
			try {
				setTitle("I'm listening on "+ InetAddress.getLocalHost().getHostAddress()+":"+portNumber.getText());
	...
	Action Connect = new AbstractAction("Connect") {
		public void actionPerformed(ActionEvent e) {
			...
			dec.connectToServer(ipaddress.getText(),Integer.parseInt(portNumber.getText()));
			changed = false;
			Save.setEnabled(false);
			SaveAs.setEnabled(false);
		}
	...
	public class InputChecker implements Runnable{
		public void run() {
			while (true){
				if (dec.isConnected()){dec.startStream();}
				dec.checkForInput();
				try {
					Thread.sleep(0);
				}
	...
	public void createChecker(){
		new Thread(new InputChecker()).start();
	}
